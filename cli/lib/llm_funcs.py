...
def rag_output(query, docs):
    response = client.models.generate_content(
        ...
        (Output SHOULD NOT contain any metadata like id, scores etc.)
...
def llm_summary_with_citations(query, docs):
    response = client.models.generate_content(
        model=model,
...
def llm_question_answer(question, context):
    response = client.models.generate_content(
        model=model,
